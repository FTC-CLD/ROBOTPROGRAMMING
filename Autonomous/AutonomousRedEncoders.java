...
public class AutonomousRedEncoders extends LinearOpMode  {
    ...
    public void GrabStone(int stone) {
        r.DriveEncode( -stone*r.LStone, 80 ,0.3,1);
        r.DriveStop();
        r.manageExtender.enterNext();
        while (opModeIsActive() && r.manageExtender.running()) {
            r.BasicLoopTele();
        } 
        r.DriveEncodeRamp( -stone*r.LStone, 67,0.24,0.32,1);

    }
    public int GrabSkystone() {
        int stone = 0;
        ...
        stone++;
        r.DriveEncode(-stone*r.LStone,68, 0.27,0);
        if (r.IsSkystone()) {
            r.DriveStop();
            GrabStone(stone);
            return 4;
        ...
        stone++;
        r.DriveEncode(-stone*r.LStone,68, 0.27,2);
        GrabStone(stone);
        return 0;
    }
        @Override
